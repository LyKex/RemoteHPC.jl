...
function pull(server::Server, remote::String, loc::String)
    ...
    if islocal(server)
        ...
        if filesize(server, remote) > 100e6 || isdir(server, remote)
            ...
            out = Pipe()
            err = Pipe()
            remote_src = isdir(server, remote) ? remote * "/" : remote
            run(pipeline(`rsync -a $(ssh_string(server)):$remote_src $path`; stdout=out, stderr=err))
            close(out.in)
            close(err.in)
...
function push(filename::String, server::Server, server_file::String)
    if islocal(server)
        ...
        out = Pipe()
        err = Pipe()
        src = isdir(filename) ? filename * "/" : filename
        run(pipeline(`rsync -a $src $server_file`; stdout=out, stderr=err))
        close(out.in)
        close(err.in)
        stderr_str = read(err, String)
        isempty(stderr_str) || error(stderr_str)
    elseif !isdir(filename) && filesize(filename) <= 100e6 && isalive(server)
        ...
        err = Pipe()
        src = isdir(filename) ? filename * "/" : filename
        run(pipeline(`rsync -a $src $(ssh_string(server)):$server_file`; stdout=out, stderr=err))
        close(out.in)
        close(err.in)
        stderr_str = read(err, String)
        isempty(stderr_str) || error(stderr_str)
